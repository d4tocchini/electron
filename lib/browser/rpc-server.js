...
handleRemoteCommand('ELECTRON_BROWSER_REQUIRE', function (event, contextId, name) {
  const customEvent = new CustomEvent()
  event.sender.emit('remote-require', customEvent, name)
  ...
  if (customEvent.defaultPrevented) {
    ...
    customEvent.returnValue = process.mainModule.require(name)
  }
...
handleRemoteCommand('ELECTRON_BROWSER_GLOBAL', function (event, contextId, name) {
  const customEvent = new CustomEvent()
  event.sender.emit('remote-get-global', customEvent, name)
  ...
  if (customEvent.defaultPrevented) {
    ...
    customEvent.returnValue = global[name]
  }
